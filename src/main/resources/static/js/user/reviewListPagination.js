...
(function() {
    fetch(`/api/user/myReviewList?page=0`)
        .then((res)=>res.json())
        .then((data)=>{
            renderPagination(data.reviewList[0].totalPages);
        })
})();
...
function renderPagination(currentPage) {
    fetch(`/api/user/myReviewList?page=0`)
        ...
        .then((data)=>{
            //총 페이지 수
            const total = Math.ceil(data.reviewList[0].totalElements/10);
            //화면에 보여질 페이지 그룹
