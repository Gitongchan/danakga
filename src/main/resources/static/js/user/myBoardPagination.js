...
const pagenation = document.querySelector('.mypage_article .text-center .pagination');
const myBoardList = document.getElementById('myBoardTableList');

function freeRender(url) {
    fetch(`${url}page=0`)
        .then((res) => res.json())
        .then((data) => {
            pagenation.innerHTML='';
            renderFreePagination(data.lists[0].totalPage,url);
        })
}

function qnaRender(url) {
    fetch(`${url}page=0`)
        .then((res) => res.json())
        .then((data) => {
            pagenation.innerHTML='';
            renderQnaPagination(data.comments[0].totalPage,url);
        })
}

function renderFreePagination(currentPage,url) {
    fetch(`${url}page=0`)
        .then((res)=>res.json())
        .then((data)=>{
            //총 페이지 수
            ...
            for (const id of setnum) {
                id.addEventListener('click', function(e) {
                    ...
                    //페이지 그리는 함수
                    else onFreeList(id,url);



                    pagenation.innerHTML = "";
                    renderFreePagination(selectedPage,url);//페이지네이션 그리는 함수
...
function renderQnaPagination(currentPage,url) {
    fetch(`${url}page=0`)
        .then((res)=>res.json())
        .then((data)=>{
            //총 페이지 수
            ...
            for (const id of setnum) {
                id.addEventListener('click', function(e) {
                    ...
                    //페이지 그리는 함수
                    else onQnaList(id,url);



                    pagenation.innerHTML = "";
                    renderQnaPagination(selectedPage,url);//페이지네이션 그리는 함수
